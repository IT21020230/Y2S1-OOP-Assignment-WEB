        ...
        <main class="contact-page ptb100">
            <div class="container">
                    ...
                    <div class="col-md-8 col-sm-12">                       
                        ...
                        <form action="tvseriescontrol" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                            	<label for="movie-input-field">Celebrity</label>
                            	<select class="custom-select" name="selectCelebrity">
                            	<%                       
					try{
						connection = DBConnectUtil.getConnection();
						statement=connection.createStatement();
						String tvShowID = request.getParameter("tvshow_id");
						String sql ="select c.celebrity_id, c.name from celebritytb c where c.celebrity_id not in (select tc.celebrity from tvshowcelebritytb tc where tc.tvshow="+tvShowID+")";
						resultSet = statement.executeQuery(sql);
						while(resultSet.next()){
					%>
  									<option value="<%=resultSet.getString("celebrity_id")%>"><%=resultSet.getString("name")%></option>
  						<%
						}
						connection.close();
					} catch (Exception e) {
				...
				%>
							...
							<input type="hidden" name="txtTVShowID" value="<%=request.getParameter("tvshow_id")%>">
							
                            <!-- Form Group -->
                            <div class="form-group text-center">
                                <button class="btn btn-main btn-effect" type="submit" name="btnAddCast" value="updateMovie">Add Cast</button>
                            </div>
                        </form>
